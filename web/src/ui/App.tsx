...
function readPage():Page{return pageFromHash()??'season';}
export function App({repository}:{repository:LeagueRepository}){
  const [snapshot,setSnapshot]=useState(emptySnapshot),[page,setPage]=useState<Page>(readPage),[sectionOpen,setSectionOpen]=useState(()=>pageFromHash()!==null),[dialog,setDialog]=useState<DialogState>(null);
  ...
  const generation=useRef(0),authEpoch=useRef(0),detailRequest=useRef(0),working=useRef(false),navRef=useRef<HTMLElement>(null);
  ...
  useEffect(()=>{
    const syncLocation=()=>{detailRequest.current++;setDetailBusy(false);const next=pageFromHash();setPage(next??'season');setSectionOpen(next!==null);setDialog(null);};
  ...
  useEffect(()=>{if(pageFromHash()!==null)requestAnimationFrame(()=>navRef.current?.scrollIntoView({block:'start'}));},[]);
  ...
  const returnHome=()=>{
    detailRequest.current++;setDetailBusy(false);setSectionOpen(false);setPage('season');setDialog(null);
  ...
  return <>{sectionOpen&&<a className="skip" href="#main-content">Skip to content</a>}
    ...
    <nav ref={navRef} className="main-nav stone-nav" aria-label="League navigation">
      {pages.map(({id,label,disabled})=>disabled?
        ...
        <a key={id} className={'nav-item '+(sectionOpen&&page===id?'active':'')} href={'#'+id} aria-current={sectionOpen&&page===id?'page':undefined} onClick={e=>{e.preventDefault();openSection(id);}}><span>{label}</span></a>)}
    </nav>
    {sectionOpen&&<div className="content-region"><div className="content-shell">
