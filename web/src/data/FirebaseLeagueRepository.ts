...
export class FirebaseLeagueRepository implements LeagueRepository {
  ...
  async load():Promise<LeagueSnapshot>{
    ...
    const [bootstrap,directory]=await Promise.all([
      ...
      this.call<{players:PlayerRecord[];events:EventRecord[];matches:LeagueSnapshot['matches'];enteredSeason:boolean}>('getPlayerSiteDirectory')
    ]);
    return {membership:'ACTIVE',viewer:bootstrap.viewer,season:bootstrap.activeSeason,standings:bootstrap.leaderboard,players:directory.players,matches:directory.matches,enteredSeason:directory.enteredSeason,events:directory.events.map(e=>e.eventId===bootstrap.upcomingEvent?.eventId?{...e,...bootstrap.upcomingEvent}:e)};
  }
