...
async function authRequest(endpoint, body) {
  ...
  if (!response.ok) throw new Error("Auth " + endpoint + " failed: " + JSON.stringify(payload));
  return payload;
}
...
function parseFixturePlayers(fixturePath) {
  ...
  const result = spawnSync(pythonExecutable, [parserPath, path.resolve(fixturePath)], {
    encoding: "utf8",
    windowsHide: true,
  });
  if (result.error) {
  ...
  if (players.length !== 2) {
    throw new Error("Warmup fixture must contain exactly two source players; found " + players.length + ".");
  }
  if (players.some((player) => !player.sourceName || !String(player.sourceName).trim())) {
    throw new Error("Warmup fixture source players must have names for automatic league mapping.");
  }
...
function runIngestion(matchId, fixturePath, outputRoot) {
  const result = spawnSync(process.execPath, [
    ...
    "--match", matchId,
    "--game", "G1",
    "--file", path.resolve(fixturePath),
    "--python", pythonExecutable,
    "--firestore-port", firestorePort,
    "--canonical-root", outputRoot,
  ], {
    stdio: "inherit",
    windowsHide: true,
  });
  ...
  if (result.status !== 0) {
    throw new Error("Replay ingestion failed with exit code " + result.status + ".");
  }
}
...
try {
  ...
  if (!league.activeSeason?.seasonId) {
    throw new Error("An active emulator Season is required. Run scripts/create-emulator-season.mjs first.");
  }
  ...
  console.log("Creating 1v1 warmup Event with civilization drafting disabled...");
  const created = await callCallable("adminCreateEvent", adminToken, {
    requestId: randomUUID(),
    title: "Replay Warmup — " + runTag,
    description: "Mock 1v1 warmup used to exercise replay ingestion and Battle Statistics.",
    startsAt,
    endsAt,
    signupDeadlineAt,
    checkInOpensAt,
  ...
  for (let index = 0; index < replayPlayers.length; index += 1) {
    ...
    const user = await createTestUser("warmup-" + runTag + "-" + (index + 1) + "@league.local");
    ...
    await callCallable("adminSetMembershipStatus", adminToken, {
      ...
      status: "ACTIVE",
      reason: "Warmup replay fixture integration test",
    });
    ...
    console.log("  slot " + replayPlayer.replaySlot + " -> " + replayPlayer.sourceName + " -> " + membership.playerId);
  }
  ...
  if (generated.matches?.length !== 1 || generated.matches[0]?.format !== "ONE_V_ONE") {
    throw new Error("Warmup planner did not produce exactly one ONE_V_ONE Match.");
  }
  ...
  if (!Array.isArray(approved.officialMatchIds) || approved.officialMatchIds.length !== 1) {
    throw new Error("Warmup approval did not produce exactly one official Match.");
  }
  ...
  const [gameBefore, draftDocument] = await Promise.all([
    ...
    readDocument("matches/" + matchId + "/civilizationDrafts/G1", adminToken, true),
  ]);

  if (stringField(gameBefore, "civilizationDraftId") !== null) {
    throw new Error("Warmup Game unexpectedly references a civilization draft.");
  }
  if (draftDocument !== null) {
    throw new Error("Warmup Match unexpectedly created a civilization draft document.");
  }
  ...
  const gameAfterFirst = await readDocument("matches/" + matchId + "/games/G1", adminToken);
  const firstRawStatsId = stringField(gameAfterFirst, "activeRawStatsId");
  const firstMatchStatisticsId = stringField(gameAfterFirst, "activeMatchStatisticsId");
  const firstRawRevision = numberValue(gameAfterFirst.fields?.rawStatsRevision);
  const firstStatisticsRevision = numberValue(gameAfterFirst.fields?.matchStatisticsRevision);

  if (!firstRawStatsId || !firstMatchStatisticsId || firstRawRevision !== 1 || firstStatisticsRevision !== 1) {
    throw new Error("First POV did not create raw replay evidence and Canonical Match Statistics revision 1.");
  }

  const firstStatistics = await readDocument(
    "matches/" + matchId + "/games/G1/matchStatistics/" + firstMatchStatisticsId,
    adminToken,
  );
  if (stringField(firstStatistics, "statisticsProjectionVersion") !== "AOF_CANONICAL_STATISTICS_V1") {
    throw new Error("First POV stored the wrong Canonical Statistics projection version.");
  }

  console.log("Ingesting second POV fixture as the active replay/statistics revision...");
  runIngestion(matchId, fixtureTwo, outputRoot);

  const gameAfterSecond = await readDocument("matches/" + matchId + "/games/G1", adminToken);
  const secondRawStatsId = stringField(gameAfterSecond, "activeRawStatsId");
  const secondMatchStatisticsId = stringField(gameAfterSecond, "activeMatchStatisticsId");
  const secondRawRevision = numberValue(gameAfterSecond.fields?.rawStatsRevision);
  const secondStatisticsRevision = numberValue(gameAfterSecond.fields?.matchStatisticsRevision);

  if (!secondRawStatsId || secondRawStatsId === firstRawStatsId || secondRawRevision !== 2) {
    throw new Error("Second POV did not supersede raw replay evidence as revision 2.");
  }
  if (!secondMatchStatisticsId || secondMatchStatisticsId === firstMatchStatisticsId || secondStatisticsRevision !== 2) {
    throw new Error("Second POV did not supersede Canonical Match Statistics as revision 2.");
  }

  const [preservedFirst, activeSecond] = await Promise.all([
    readDocument("matches/" + matchId + "/games/G1/matchStatistics/" + firstMatchStatisticsId, adminToken),
    readDocument("matches/" + matchId + "/games/G1/matchStatistics/" + secondMatchStatisticsId, adminToken),
  ]);

  if (!preservedFirst) {
    throw new Error("Superseded Canonical Match Statistics revision 1 was not preserved.");
  }
  if (stringField(activeSecond, "supersedesMatchStatisticsId") !== firstMatchStatisticsId) {
    throw new Error("Active Canonical Match Statistics revision 2 does not point to revision 1.");
  }
  if (stringField(activeSecond, "contractVersion") !== "AOF_CANONICAL_MATCH_STATISTICS_V1") {
    throw new Error("Active Canonical Match Statistics uses the wrong ingestion contract.");
  }

  const statisticsFields = mapFields(activeSecond, "statistics");
  const projectedParticipants = statisticsFields.participants?.arrayValue?.values ?? [];
  if (projectedParticipants.length !== 2) {
    throw new Error("Active Canonical Match Statistics does not contain exactly two projected participants.");
  }

  for (const value of projectedParticipants) {
    const participant = value.mapValue?.fields ?? {};
    for (const field of ["opening", "economy", "combat", "mapPresence", "observedCommands", "selectionEvidence"]) {
      if (!participant[field]?.mapValue) {
        throw new Error("Projected participant is missing Battle statistics field " + field + ".");
      }
    }
  }

  if (arrayValues(activeSecond, "playerMapping").length !== 2) {
    throw new Error("Active Canonical Match Statistics does not retain the two-player league mapping.");
  }
  ...
  console.log("  POV 1 raw revision: " + firstRawRevision + " | statistics revision: " + firstStatisticsRevision);
  console.log("  POV 2 raw revision: " + secondRawRevision + " | statistics revision: " + secondStatisticsRevision);
  console.log("  Active projection: AOF_CANONICAL_STATISTICS_V1");
  console.log("  Canonical artifacts: " + outputRoot);
  console.log("  Test players: " + testPlayers.map((player) => player.steamName).join(" vs "));
} catch (error) {
