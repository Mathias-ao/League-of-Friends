...
function runPythonJson(scriptPath, scriptArgs, label) {
  const result = spawnSync(pythonExecutable, [scriptPath, ...scriptArgs], {
    encoding: "utf8",
    windowsHide: true,
  });

  if (result.error) {
    throw new Error("Could not start " + label + " with '" + pythonExecutable + "': " + result.error.message);
  }
  if (result.status !== 0) {
...
function runReplayPipeline() {
  const parserPath = path.resolve("replay-tools", "parse_replay.py");
  const statisticsProjectorPath = path.resolve("replay-tools", "statistics_projector.py");
  const replayPath = path.resolve(replayFile);
  const canonicalDir = path.resolve(
    canonicalRoot,
    safePathSegment(matchId),
    safePathSegment(gameId),
    safePathSegment(path.basename(replayFile, path.extname(replayFile))) + "-" + randomUUID(),
    "canonical",
  );

  const parsed = runPythonJson(
    parserPath,
    [replayPath, "--canonical-dir", canonicalDir],
    "Replay parser",
  );
  const statistics = runPythonJson(
    statisticsProjectorPath,
    [canonicalDir],
    "Canonical statistics projector",
  );

  if (statistics?.source?.replaySha256 !== parsed.sourceHash) {
    throw new Error(
      "Canonical statistics source hash " + (statistics?.source?.replaySha256 ?? "(missing)") +
      " does not match parser source hash " + parsed.sourceHash + ".",
    );
  }

  return { parsed, statistics, canonicalDir };
}
...
try {
  ...
  console.log("Adapter schema: " + parsed.schemaVersion);
  console.log("Canonical schema: " + (statistics.source?.canonicalSchemaVersion ?? "?"));
  console.log("Statistics projection: " + statistics.statisticsProjectionVersion);
  ...
  console.log("Canonical evidence: " + canonicalDir);
  console.log("Replay players:");
  for (const player of parsed.payload?.players ?? []) {
    console.log(
      "  slot " + player.replaySlot + ": " + player.name +
      " | team " + (player.teamId ?? "?") + " | civ " + (player.civilizationId ?? "?"),
    );
  }
  ...
  if (dryRun) {
    console.log("Dry run complete; CanonicalReplay/statistics were generated locally but Firestore was not changed.");
    process.exit(0);
  }

  const rawResult = await callCallable("adminIngestReplayStats", token, {
  ...
  console.log("Replay evidence ingested: " + JSON.stringify(rawResult));
  console.log("Active raw-stat revision: " + rawResult.rawStatsRevision);
  console.log("rawStatsId: " + rawResult.rawStatsId);

  const statisticsResult = await callCallable("adminIngestCanonicalMatchStatistics", token, {
    requestId: randomUUID(),
    matchId,
    gameId,
    sourceHash: parsed.sourceHash,
  ...
  console.log("Canonical Battle statistics ingested: " + JSON.stringify(statisticsResult));
  console.log("Active Match Statistics revision: " + statisticsResult.matchStatisticsRevision);
  console.log("matchStatisticsId: " + statisticsResult.matchStatisticsId);
  console.log("Canonical evidence retained locally: " + canonicalDir);
} catch (error) {
