...
| Economy statistics | `AOF_ECONOMY_STATISTICS_V2` | Projects queue-derived Villager checkpoints, TC placement/activity, eco-tech, Farm, market, food-animal interaction and 20-minute commitment metrics while retaining evidence boundaries. |
...
Economic technology output uses an explicit DE economic-tech set including Coinage, Banking, Guilds and Caravan. V2 uses the first observed request for ordinary economic technologies (the latest-click rule remains specific to age advances), while retaining nominal research duration as a separate inferred completion. TownBell-compatible `eco upgrades by Castle` excludes Loom and uses the Castle click as its boundary. `Farms before Horse Collar` uses the first Horse Collar click; `Farms before Castle` uses the Castle click.
