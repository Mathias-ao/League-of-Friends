...
from skirmish_detector import SKIRMISH_MODEL_VERSION, detect_skirmishes
...
class SkirmishDetectorTests(unittest.TestCase):
    ...
    def test_later_created_target_is_attributed_from_prior_control_selection(self):
        result = detect_skirmishes(
            manifest=manifest(),
            initial_objects=initial_objects(),
            action_events=[
        ...
        self.assertIn("targeted_controlled_object", direct[0]["evidenceMethods"])
        self.assertEqual(direct[0]["confidence"], "high")

    def test_one_sided_targeted_order_is_skirmish_but_not_reciprocal_pair(self):
        result = detect_skirmishes(
            manifest=manifest(),
            initial_objects=initial_objects(),
            action_events=[
